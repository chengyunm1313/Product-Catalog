...
function extractTextFromJson(json: Record<string, unknown> | null): string {
	...
	const walk = (node: Record<string, unknown>): string => {
		...
		if (Array.isArray(node.content)) {
			return node.content.map((child: Record<string, unknown>) => walk(child)).join('\n');
		}
		return '';
	};
		...
		const body = {
			...
			contentJson: {
				...
				content: [
					{
						type: 'paragraph',
						content: [{ type: 'text', text: form.get('content') || '' }],
					},
				],
			},
			seo: {
		...
		<div className={styles.formPage}>
			...
			<form onSubmit={handleSubmit}>
				...
				<div className={styles.formGroup}>
					...
					<textarea
						name='content'
						defaultValue={extractTextFromJson(post.contentJson as Record<string, unknown> | null)}
						rows={8}
					/>
				</div>
